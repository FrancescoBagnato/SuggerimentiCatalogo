    ...
    el.innerHTML = sorted.map(req => `
        <div class="req-card">
            ...
            <div class="req-meta">
                ...
                <span>📅 ${req.timestamp ? new Date(req.timestamp).toLocaleDateString('it-IT', { day: '2-digit', month: '2-digit', year: 'numeric' }) : esc((req.date || '').split(',')[0])}</span>
            </div>
...
function requestTypeLabel(req) {
    if (req.type !== 'Serie TV') return req.type || '';
    const seasons = Array.isArray(req.seasons)
        ? req.seasons.map(Number).filter(n => Number.isInteger(n) && n >= 1 && n <= 20).sort((a, b) => a - b)
        : [];
    return seasons.length ? `Serie TV - ${seasons.join(', ')}` : 'Serie TV';
}

function requestTypeBadge(req) {
    return `<span class="badge badge-type">${esc(requestTypeLabel(req))}</span>`;
}

const requestTypeSelect = document.getElementById('type');
const requestSeasonsField = document.getElementById('requestSeasonsField');
const requestSeasonCheckboxes = () => Array.from(document.querySelectorAll('input[name="requestedSeason"]'));

function updateRequestSeasonField() {
    const isSeries = requestTypeSelect?.value === 'Serie TV';
    if (requestSeasonsField) requestSeasonsField.hidden = !isSeries;
    if (!isSeries) requestSeasonCheckboxes().forEach(input => { input.checked = false; });
}

requestTypeSelect?.addEventListener('change', updateRequestSeasonField);
updateRequestSeasonField();
...
document.getElementById('requestForm').addEventListener('submit', async function(e) {
    ...
    const seasons = requestTypeSelect?.value === 'Serie TV'
        ? requestSeasonCheckboxes().filter(input => input.checked).map(input => Number(input.value)).sort((a, b) => a - b)
        : [];
    ...
    else if (typeVal === 'Serie TV' && !seasons.length) reqError = 'Seleziona almeno una stagione.';
    if (reqError) {

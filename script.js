...
function getNickname() {
  return localStorage.getItem("catalogNick") || "";
}
function saveNickname(n) {
  if (n) localStorage.setItem("catalogNick", n.trim());
}

// Voti consigliati: { id: score } salvati in localStorage
function getSuggRatings() {
  return JSON.parse(localStorage.getItem("suggRatings") || "{}");
}
function saveSuggRating(id, score) {
  const r = getSuggRatings();
  r[id] = score;
  localStorage.setItem("suggRatings", JSON.stringify(r));
}
function getSuggRating(id) {
  return getSuggRatings()[id] || 0;
}
function hasSuggVoted(id) {
  return !!getSuggRatings()[id];
}
...
function starSvgInline(type, sz) {
  ...
  if (type === "full")
    return (
      base +
      `<polygon points="${pts}" fill="#fbbf24" stroke="#fbbf24" stroke-width="1.5" stroke-linejoin="round"/></svg>`
    );
  if (type === "empty")
    return (
      base +
      `<polygon points="${pts}" fill="none" stroke="#64748b" stroke-width="1.5" stroke-linejoin="round"/></svg>`
    );
  return (
    base +
...
async function enableAdmin() {
  ...
  try {
    ...
    if (!storedHash) {
      alert("Hash admin non configurato su Firebase.");
      return;
    }
    ...
    alert("Errore verifica password: " + e.message);
  }
}
...
function renderEvased(list) {
  ...
  const sorted = [...list].sort(
    (a, b) => b.evadedTimestamp - a.evadedTimestamp,
  );
  el.innerHTML = sorted
    .map(
      (req) => `
...
document.querySelectorAll(".tab[data-sort]").forEach((tab) => {
  tab.addEventListener("click", function () {
    document
      .querySelectorAll(".tab[data-sort]")
...
function openCatalogPopup(title, parentTitle = null) {
  ...
  const watchList = Object.entries(users).filter(
    ([, u]) => u.status === "watching",
  );
        ...
        <div class="popup-box" id="popupBox">
            ...
            <div class="popup-stars" id="popupStars">
                ${Array.from({ length: 10 }, (_, i) => {
                  ...
                  return (
                    `<button class="star-btn ${cls}" data-val="${val}" title="${val}/10">` +
  ...
  overlay.querySelectorAll(".status-btn").forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      overlay
        ...
        .forEach((b) => b.classList.remove("active-seen", "active-watching"));
      if (popupSelectedStatus === "seen") btn.classList.add("active-seen");
      if (popupSelectedStatus === "watching")
        btn.classList.add("active-watching");
    });
  });

  overlay
    .querySelector("#popupSave")
    .addEventListener("click", saveCatalogEntry);
...
async function saveCatalogEntry() {
  ...
  try {
    if (!popupSelectedStatus) {
      ...
      await set(userRef, {
        status: popupSelectedStatus,
        rating: popupSelectedRating || null,
        updatedAt: Date.now(),
      });
    }
    closePopup();
...
function updateFolderUI(folder) {
  ...
  const avg = allRatings.length
    ? Math.round(
        (allRatings.reduce((a, b) => a + b, 0) / allRatings.length) * 2,
      ) / 2
    : null;
  ...
  if (!avatarDiv) {
    ...
    avatarDiv.className = "ci-avatars";
    folder.insertBefore(avatarDiv, folder.querySelector(".ci-folder-list"));
  }
  avatarDiv.innerHTML = [
    ...[...seenNicks].map(
      (n) =>
        `<div class="ci-avatar seen" title="${esc(n)} — Visto">${esc(n[0].toUpperCase())}</div>`,
    ),
    ...[...watchNicks].map(
      (n) =>
        `<div class="ci-avatar watching" title="${esc(n)} — In corso">${esc(n[0].toUpperCase())}</div>`,
    ),
  ].join("");
}
...
function updateCatalogItemUI(li) {
  ...
  const avg = avgRating(users);
  const seenUsers = Object.entries(users).filter(
    ([, u]) => u.status === "seen",
  );
  const watchUsers = Object.entries(users).filter(
    ([, u]) => u.status === "watching",
  );
...
onValue(catalogRef, (snap) => {
  ...
  document
    .querySelectorAll(".catalog-item:not(.catalog-folder)")
    .forEach((li) => updateCatalogItemUI(li));
  // Aggiorna cartelle (media dai sub-item)
  document
    .querySelectorAll(".catalog-folder")
    .forEach((folder) => updateFolderUI(folder));
});
...
async function voteSuggested(id, score) {
  const myNick = getNickname();
  if (!myNick) {
    alert("Imposta prima il tuo Nickname!");
    return;
  }
  const item = allSuggested.find((s) => s.id === id);
  if (!item) return;
  if (item.nick === myNick) {
    alert("Non puoi votare un tuo consiglio!");
    return;
  }
...
function renderSuggested(list) {
  ...
  if (!list.length) {
    el.innerHTML =
      '<div class="sugg-empty">Nessun consiglio ancora.<br>Sii il primo!</div>';
    return;
  }
  ...
  el.querySelectorAll(".sugg-delete").forEach((btn) => {
    btn.addEventListener("click", async () => {
      ...
      try {
        await remove(ref(db, "suggested/" + btn.dataset.id));
      } catch (e) {
        alert("Errore: " + e.message);
      }
    });
...
document.getElementById("suggSubmit").addEventListener("click", async () => {
  ...
  btn.querySelector(".btn-submit-text").textContent = "Invio…";
  try {
    await push(suggestedRef, { title, nick, votes: 0, timestamp: Date.now() });
    titleEl.value = "";
  } catch (e) {
    alert("Errore: " + e.message);
  } finally {
...
document.querySelectorAll("[data-sugg-sort]").forEach((tab) => {
  tab.addEventListener("click", function () {
    document
      .querySelectorAll("[data-sugg-sort]")
      .forEach((t) => t.classList.remove("active"));
    this.classList.add("active");
...
function renderCatalogFromFirebase() {
  ...
  if (!tv.length && !film.length) {
    container.innerHTML =
      '<div class="catalog-empty">Nessun titolo nel catalogo.<br>Aggiungili dal pannello admin → 🗂️ Catalogo.</div>';
    return;
  }
...
function attachCatalogEvents() {
  ...
  document.querySelectorAll(".catalog-cat-btn").forEach((btn) => {
    ...
    btn.addEventListener("click", function () {
      ...
      document
        .querySelectorAll(".catalog-panel")
        .forEach((p) => p.classList.remove("open"));
      document
        .querySelectorAll(".catalog-cat-btn")
  ...
  document
    .querySelectorAll(
      ".catalog-item:not(.catalog-folder):not(.catalog-subitem)",
    )
    .forEach((li) => {
        ...
        cat.querySelectorAll(".catalog-item").forEach((li) => {
          ...
          if (match) {
            ...
            li.querySelector(".ci-name").innerHTML =
              esc(plain.slice(0, idx)) +
              "<mark>" +
              esc(plain.slice(idx, idx + q.length)) +
              "</mark>" +
              esc(plain.slice(idx + q.length));
          } else {
...
function updateAllCategoryCounters() {
  ["serietv", "film"].forEach((key) => {
    ...
    if (!panel) return;
    if (countEl) {
      const singles = panel.querySelectorAll(
        ".catalog-item:not(.catalog-folder):not(.catalog-subitem)",
      ).length;
...
function openAdminPanel() {
  ...
  const catalogItems = Array.from(
    ...
    category:
      li.closest(".catalog-category")?.querySelector(".cat-name")
        ?.textContent || "",
  }));
  ...
  overlay.querySelectorAll(".admin-tab").forEach((tab) => {
    tab.addEventListener("click", function () {
      ...
      this.classList.add("active");
      overlay
        .querySelector("#ap-" + this.dataset.panel)
  ...
  function renderApRequests() {
    ...
    if (!allRequests.length) {
      el.innerHTML = '<div class="ap-empty">Nessuna richiesta.</div>';
      return;
    }
    ...
    el.querySelectorAll('.ap-btn-del[data-type="request"]').forEach((btn) => {
      btn.addEventListener("click", async () => {
        if (!confirm(`Eliminare definitivamente "${btn.dataset.title}"?`))
          return;
        try {
          await remove(ref(db, "requests/" + btn.dataset.id));
          renderApRequests();
        } catch (e) {
          alert("Errore: " + e.message);
        }
  ...
  function renderApEvased() {
    ...
    if (!allEvased.length) {
      el.innerHTML = '<div class="ap-empty">Nessuna richiesta evasa.</div>';
      return;
    }
    const sorted = [...allEvased].sort(
      (a, b) => b.evadedTimestamp - a.evadedTimestamp,
    );
    el.innerHTML = sorted
      .map(
    ...
    el.querySelectorAll(".ap-btn-del").forEach((btn) => {
      btn.addEventListener("click", async () => {
        ...
        try {
          await remove(ref(db, "evased/" + btn.dataset.id));
          renderApEvased();
        } catch (e) {
          alert("Errore: " + e.message);
        }
  ...
  function renderApSugg() {
    ...
    if (!allSuggested.length) {
      el.innerHTML = '<div class="ap-empty">Nessun consiglio.</div>';
      return;
    }
    const sorted = [...allSuggested].sort((a, b) =>
      a.title.localeCompare(b.title, "it"),
    );
    el.innerHTML = sorted
      .map(
  ...
  function getFilteredItems(filterCat, searchQ) {
    ...
    if (!filterCat || filterCat === "cat-serietv") {
      (catalogStructure.serietv || []).forEach((i) =>
        items.push({ ...i, _cat: "serietv", _catLabel: "Serie TV" }),
      );
    }
    if (!filterCat || filterCat === "cat-film") {
      (catalogStructure.film || []).forEach((i) =>
        items.push({ ...i, _cat: "film", _catLabel: "Film" }),
      );
    }
    if (searchQ) {
  ...
  async function saveStructureToFirebase() {
    try {
      ...
      alert("Errore salvataggio Firebase: " + e.message);
    }
  }
  ...
  function renderApCatalog(filterCat = "", searchQ = "") {
    ...
    if (!items.length) {
      el.innerHTML = '<div class="ap-empty">Nessun risultato.</div>';
      return;
    }
    ...
    el.querySelectorAll(".ap-btn-del").forEach((btn) => {
      btn.addEventListener("click", async () => {
        ...
        catalogStructure[cat] = catalogStructure[cat].filter(
          (i) => i.title !== title,
        );
        await saveStructureToFirebase();
        renderApCatalog(
          overlay.querySelector(".ap-filter-btn.active")?.dataset.filter || "",
          overlay.querySelector("#apCatSearch")?.value || "",
        );
      });
    });
    ...
    el.querySelectorAll(".ap-subitem-del").forEach((btn) => {
      btn.addEventListener("click", async (e) => {
        ...
        if (folder)
          folder.children = (folder.children || []).filter((s) => s !== sub);
        await saveStructureToFirebase();
        renderApCatalog(
          overlay.querySelector(".ap-filter-btn.active")?.dataset.filter || "",
          overlay.querySelector("#apCatSearch")?.value || "",
  ...
  function refreshFolderSelect() {
    ...
    sel.innerHTML = allFolders.length
      ? allFolders
          .map(
            (f) =>
              `<option value="${esc(f.title)}" data-cat="${f._cat}">${esc(f.title)}</option>`,
          )
          .join("")
  ...
  overlay
    .querySelector("#apImportCatalog")
    ?.addEventListener("click", async () => {
      if (
        !confirm(
          "Importare tutti i titoli originali su Firebase?\nQuesto sovrascrive la struttura attuale.",
        )
      )
        return;
      const importData = {
        serietv: [
          ...
          {
            type: "folder",
            title: "Avvocato di Difesa",
            children: ["Stagione 1", "Stagione 2", "Stagione 3", "Stagione 4"],
          },
          {
            type: "folder",
          ...
          { type: "folder", title: "Barry", children: ["Stagione 1"] },
          {
            type: "folder",
            title: "Better Call Saul",
            children: ["Stagione 1", "Stagione 2"],
          },
          {
            type: "folder",
          ...
          { type: "folder", title: "Black Bird", children: ["Stagione 1"] },
          {
            type: "folder",
            title: "Black Mirror",
            children: ["Stagione 1", "Stagione 2"],
          ...
          { type: "folder", title: "Chernobyl", children: ["Stagione 1"] },
          {
            type: "folder",
            title: "Chiamatemi Anna",
            children: ["Stagione 1"],
          },
          {
            type: "folder",
            title: "DAHMER - Mostro: La storia di Jeffrey Dahmer",
            children: ["Stagione 1"],
          },
          {
            type: "folder",
          ...
          { type: "folder", title: "Domina", children: ["Stagione 1"] },
          {
            type: "folder",
            title: "Dove nessuno guarda - Il caso di Elisa Claps",
            children: ["Stagione 1"],
          },
          { type: "folder", title: "Downton Abbey", children: ["Stagione 1"] },
          { type: "folder", title: "Dune Prophecy", children: ["Stagione 1"] },
          {
            type: "folder",
            title: "Euphoria",
            children: ["Speciale", "Stagione 1", "Stagione 2"],
          },
          {
            type: "folder",
            title: "Fallout",
          ...
          { type: "folder", title: "Fargo", children: ["Stagione 1"] },
          { type: "folder", title: "Fleabag", children: ["Stagione 1"] },
          {
            type: "folder",
            title: "Fondazione",
            children: ["Stagione 1", "Stagione 2", "Stagione 3"],
          ...
          { type: "folder", title: "I Soprano", children: ["Stagione 1"] },
          {
            type: "folder",
          ...
          { type: "folder", title: "Invasion", children: ["Stagione 1"] },
          { type: "folder", title: "Inventing Anna", children: ["Stagione 1"] },
          {
            type: "folder",
          ...
          {
            type: "folder",
            title: "L'uomo nell'alto castello",
            children: ["Stagione 1"],
          },
          {
            type: "folder",
          ...
          {
            type: "folder",
            title: "Le regole del delitto perfetto",
            children: [
              "Stagione 1",
              "Stagione 2",
              "Stagione 3",
          ...
          { type: "folder", title: "Lucifer", children: ["Stagione 1"] },
          { type: "folder", title: "Lupin", children: ["Stagione 1"] },
          { type: "folder", title: "Mad Men", children: ["Stagione 1"] },
          { type: "folder", title: "Malice", children: ["Stagione 1"] },
          {
            type: "folder",
            title: "Mercoledì",
            children: ["Stagione 1", "Stagione 2"],
          ...
          { type: "folder", title: "Narcos", children: ["Stagione 1"] },
          { type: "folder", title: "New Girl", children: ["Stagione 1"] },
          {
            type: "folder",
            title: "One Piece",
            children: ["Stagione 1", "Stagione 2"],
          ...
          { type: "folder", title: "Outlander", children: ["Stagione 1"] },
          { type: "folder", title: "Ozark", children: ["Stagione 1"] },
          {
            type: "folder",
          ...
          { type: "folder", title: "Reacher", children: ["Stagione 1"] },
          { type: "folder", title: "Romulus", children: ["Stagione 1"] },
          { type: "folder", title: "Scissione", children: ["Stagione 1"] },
          { type: "folder", title: "Sense8", children: ["Stagione 1"] },
          { type: "folder", title: "Sex Education", children: ["Stagione 1"] },
          {
            type: "folder",
          ...
          { type: "folder", title: "Shogun", children: ["Stagione 1"] },
          { type: "folder", title: "Silo", children: ["Stagione 1"] },
          { type: "folder", title: "Six Feet Under", children: ["Stagione 1"] },
          {
            type: "folder",
            title: "Slow Horses",
            children: ["Stagione 1", "Stagione 2"],
          ...
          },
          {
            type: "folder",
            title: "Spartacus",
            children: ["Stagione 0", "Stagione 1", "Stagione 2", "Stagione 3"],
          },
          {
            type: "folder",
          ...
          { type: "folder", title: "Squid Game", children: ["Stagione 1"] },
          { type: "folder", title: "Station Eleven", children: ["Stagione 1"] },
          {
            type: "folder",
          ...
          { type: "folder", title: "Suits", children: ["Stagione 1"] },
          { type: "folder", title: "Suits LA", children: ["Stagione 1"] },
          {
            type: "folder",
            title: "Teen Wolf",
            children: [
          ...
          { type: "folder", title: "The Gentlemen", children: ["Stagione 1"] },
          {
            type: "folder",
          ...
          { type: "folder", title: "The Leftovers", children: ["Stagione 1"] },
          {
            type: "folder",
          ...
          { type: "folder", title: "The Night Of", children: ["Stagione 1"] },
          { type: "folder", title: "The Office", children: ["Stagione 1"] },
          {
            type: "folder",
            title: "The Pitt",
            children: ["Stagione 1", "Stagione 2"],
          },
          { type: "folder", title: "The Recruit", children: ["Stagione 1"] },
          {
            type: "folder",
            title: "The Regime - Il palazzo del potere",
            children: ["Stagione 1"],
          },
          {
            type: "folder",
          ...
          { type: "folder", title: "The Wire", children: ["Stagione 1"] },
          {
            type: "folder",
            title: "The Witcher",
            children: ["Stagione 1", "Stagione 2", "Stagione 3", "Stagione 4"],
          },
          { type: "folder", title: "The Young Pope", children: ["Stagione 1"] },
          { type: "folder", title: "Tredici", children: ["Stagione 1"] },
          { type: "folder", title: "True Detective", children: ["Stagione 1"] },
          { type: "folder", title: "Tulsa King", children: ["Stagione 1"] },
          {
            type: "folder",
            title: "Vikings",
            children: [
          ...
          { type: "folder", title: "Virgin River", children: ["Stagione 1"] },
          { type: "folder", title: "Vis a vis", children: ["Stagione 1"] },
          { type: "folder", title: "Watchmen", children: ["Stagione 1"] },
          {
            type: "folder",
            title: "Westworld",
            children: ["Stagione 1", "Stagione 2", "Stagione 3", "Stagione 4"],
          },
          { type: "folder", title: "Young Sherlock", children: ["Stagione 1"] },
        ],
        film: [
          {
            type: "folder",
          ...
          {
            type: "folder",
            title: "Assassinio sullOrient Express",
            children: [
          ...
          {
            type: "folder",
            title: "Attacco al potere",
            children: [
              "Attacco al potere 1 Olympus Has Fallen (2013)",
          ...
          },
          {
            type: "folder",
            title: "Dune",
            children: ["Dune 2021 Part 1", "Dune - Parte Due"],
          },
          {
            type: "folder",
          ...
          },
          {
            type: "folder",
            title: "Frozen",
            children: ["Frozen 1", "Frozen 2"],
          },
          {
            type: "folder",
            title: "Greenland",
            children: ["Greenland 2: Migration", "Greenland"],
          },
          {
            type: "folder",
            title: "Harry Potter",
          ...
          },
          {
            ...
            children: [
              ...
              "4. Hunger Games - Il canto della rivolta parte 2",
          ...
          },
          {
            ...
            children: [
              ...
              "Le due torri",
              "Lo Hobbit 1 Un viaggio inaspettato (2012)",
              "Lo Hobbit 2 La desolazione di Smaug (2013)",
              "Lo Hobbit 3 La battaglia delle cinque armate (2014)",
            ],
          },
          {
            type: "folder",
            title: "Indiana Jones",
            children: [
              "Indiana Jones 1 I predatori dell'arca perduta (1981)",
              "Indiana Jones 2 Il tempio maledetto (1984)",
              "Indiana Jones 3 L'ultima crociata (1989)",
              "Indiana Jones 4 Il regno del teschio di cristallo (2008)",
            ],
          },
          {
            type: "folder",
            title: "Inside Out",
            children: ["Inside Out 1", "Inside Out 2"],
          },
          {
            type: "folder",
            title: "John Wick",
          ...
          },
          {
            ...
            children: [
              "Jurassic Park (1993)",
              "Jurassic Park 2 - Il mondo perduto (1997)",
          ...
          {
            type: "folder",
            title: "Professor Langdon",
            children: ["Angeli e Demoni", "Il codice da Vinci", "Inferno"],
          },
          {
            type: "folder",
            title: "Re Leone",
          ...
          },
          {
            ...
            children: ["Sherlock Holmes 1", "Sherlock Holmes: Gioco di ombre"],
          },
          {
            type: "folder",
            title: "Shrek",
          ...
          },
          {
            ...
            title: "Star Wars",
            children: [
              "Star Wars 1 La minaccia fantasma (1999)",
              "Star Wars 2 L'attacco dei cloni (2002)",
              "Star Wars 3 La vendetta dei Sith (2005)",
              "Star Wars 4 Una nuova speranza (1977)",
              "Star Wars 5 L'impero colpisce ancora (1980)",
              "Star Wars 6 Il ritorno dello Jedi (1983)",
              "Star Wars 7 Il risveglio della Forza (2015)",
              "Star Wars 8 Gli ultimi Jedi (2017)",
            ],
          },
          {
            type: "folder",
            title: "The Croods",
            children: ["The Croods 1", "The Croods 2"],
          },
          {
            type: "folder",
            title: "The Maze Runner",
            children: [
              "1. Maze Runner - Il labirinto (2014)",
              "2. Maze Runner - La fuga (2015)",
              "3. Maze Runner - La rivelazione (2018)",
            ],
          },
          {
            type: "folder",
          ...
          },
          {
            ...
            children: [
              ...
              "Underworld 3 La ribellione dei Lycans (2009)",
              "Underworld 4 Il risveglio (2012)",
              "Underworld 5 Blood Wars (2016)",
            ],
          },
          { type: "folder", title: "Wicked", children: ["Wicked 2", "Wicked"] },
          {
            type: "folder",
            title: "Zootopia",
            children: ["Zootopia 2", "Zootopia"],
          },
          { type: "single", title: "21" },
          { type: "single", title: "Arrival (2016)" },
          { type: "single", title: "Barbie" },
          { type: "single", title: "Bird Box" },
          { type: "single", title: "Black Swan (2010)" },
          { type: "single", title: "Bohemian Rhapsody" },
          { type: "single", title: "Bugonia" },
          { type: "single", title: "Carry-On" },
          { type: "single", title: "Challengers (2024)" },
          { type: "single", title: "Damsel" },
          { type: "single", title: "Don't Look Up" },
          { type: "single", title: "Dracula di Bram Stoker (1999)" },
          {
            type: "single",
            title: "Dramma nel ciclismo: il caso Moriah Wilson",
          },
          { type: "single", title: "Edward Mani di Forbice" },
          { type: "single", title: "Elvis (2022)" },
          { type: "single", title: "Everything Everywhere All At Once (2022)" },
          { type: "single", title: "F1: Il Film" },
          { type: "single", title: "Gravity" },
          { type: "single", title: "House of Gucci" },
          { type: "single", title: "I peccatori" },
          {
            type: "single",
            title: "Il Curioso Caso Di Benjamin Button (2008)",
          },
          { type: "single", title: "Il diavolo veste Prada" },
          {
            type: "single",
            title: "Il Lato Positivo - Silver Linings Playbook",
          },
          { type: "single", title: "In Time (2011)" },
          { type: "single", title: "Inception (2010)" },
          { type: "single", title: "Interstellar" },
          { type: "single", title: "Io, robot" },
          { type: "single", title: "Jojo Rabbit (2019)" },
          { type: "single", title: "L'avvocato del diavolo (1997)" },
          { type: "single", title: "L'ultima missione: Project Hail Mary" },
          { type: "single", title: "La fabbrica di cioccolato" },
          { type: "single", title: "La grande scommessa" },
          { type: "single", title: "La teoria del tutto" },
          { type: "single", title: "Les Miserables" },
          { type: "single", title: "Minecraft" },
          { type: "single", title: "Minority Report (2002)" },
          { type: "single", title: "Mr and Mrs Smith" },
          { type: "single", title: "Ocean's 8 (2018)" },
          { type: "single", title: "Operation Fortune" },
          { type: "single", title: "Oppenheimer" },
          { type: "single", title: "Passengers (2016)" },
          { type: "single", title: "Peaky Blinders: The Immortal Man" },
          { type: "single", title: "Rampage: furia animale" },
          { type: "single", title: "Red Notice" },
          { type: "single", title: "Seven Sisters" },
          { type: "single", title: "Split (2017)" },
          { type: "single", title: "Super Mario Bros - Il film" },
          { type: "single", title: "Super Mario Galaxy - Il Film" },
          { type: "single", title: "Sweeney Todd" },
          { type: "single", title: "Teen Wolf - The Movie (2023)" },
          { type: "single", title: "Tenet (2020)" },
          { type: "single", title: "The Adam Project" },
          { type: "single", title: "The Danish Girl (2016)" },
          { type: "single", title: "The Father (2020)" },
          { type: "single", title: "The Gray Man" },
          { type: "single", title: "The Menu" },
          {
            type: "single",
            title: "The Phantom of the Opera at Royal Albert Hall",
          },
          { type: "single", title: "The Post (2017)" },
          { type: "single", title: "The Smashing Machine" },
          { type: "single", title: "The Social Network (2010)" },
          { type: "single", title: "The Whale (2022)" },
          { type: "single", title: "The Wolf of Wall Street" },
          { type: "single", title: "Titanic" },
          { type: "single", title: "Top Gun: Maverick" },
          { type: "single", title: "TÁR" },
          { type: "single", title: "Una battaglia dopo l'altra" },
          { type: "single", title: "Una poltrona per due" },
        ],
      };
      try {
        ...
        alert(
          "Import completato! " +
            importData.serietv.length +
            " serie TV e " +
            importData.film.length +
            " film.",
        );
        renderApCatalog();
      } catch (e) {
  ...
  overlay.querySelector("#apAddTitle").addEventListener("click", async () => {
    ...
    const exists = (catalogStructure[catKey] || []).find(
      (i) => i.title === title,
    );
    if (exists) {
    ...
    catalogStructure[catKey].sort((a, b) =>
      a.title.localeCompare(b.title, "it"),
    );
  ...
  overlay.querySelector("#apAddSub").addEventListener("click", async () => {
    ...
    const folder = (catalogStructure[catKey] || []).find(
      (i) => i.title === parentTitle,
    );
    if (!folder) {
      alert("Cartella non trovata.");
      return;
    }
    if ((folder.children || []).includes(subTitle)) {
      alert("Sotto-titolo già presente!");
      return;
    }
    ...
    try {
      ...
      if (snap.exists()) {
        ...
        const target = document.getElementById(
          key === "catalog-total" ? "catalogTotalNum" : key,
        );
  ...
  overlay.querySelectorAll(".ap-counter-save").forEach((btn) => {
    btn.addEventListener("click", async () => {
      ...
      try {
        await set(ref(db, "counters/" + btn.dataset.key), numOnly);
      } catch (e) {
  ...
  overlay.querySelectorAll(".ap-filter-btn").forEach((btn) => {
    btn.addEventListener("click", function () {
      overlay
        .querySelectorAll(".ap-filter-btn")
...
async function toggleWatchlist(firebaseTitle, displayTitle) {
  ...
  if (!nick) {
    alert("Inserisci prima il tuo nickname nel popup del titolo.");
    return;
  }
  const key = titleToKey(firebaseTitle);
  ...
  try {
    ...
    alert("Errore watchlist: " + e.message);
  }
}
...
function renderWatchlist(nick) {
  ...
  const items = Object.entries(watchlistData).sort(
    (a, b) => (b[1].addedAt || 0) - (a[1].addedAt || 0),
  );

  if (!items.length) {
    el.innerHTML =
      '<div class="catalog-empty">La tua watchlist è vuota.<br>Aggiungi titoli dal catalogo mettendo un ♥ al titolo scelto.</div>';
    return;
  }
  ...
  el.querySelectorAll(".watchlist-remove").forEach((btn) => {
    btn.addEventListener("click", async () => {
      try {
        await remove(
          ref(db, "watchlist/" + btn.dataset.nick + "/" + btn.dataset.key),
        );
      } catch (e) {
        alert("Errore: " + e.message);
      }
...
if (savedNick) {
  ...
  // Carica watchlist in background per avere i cuori aggiornati
